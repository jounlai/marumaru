...
| | こども版 | おとな版 |
|---|---|---|
| 語数 | 2,341語（169ラウンド） | 3,660語（179ラウンド） |
...
入り口の3枚目「For learners」から **English / 简体中文 / 繁體中文 / Tiếng Việt / 한국어**
を選べる。言語はモードとは別で、⚙メニューとホームでいつでも変えられる（`maruanagame-lang`）。

**お題・答え・表記は日本語のまま**で、そこがこの遊びの中身。訳すのは画面の文字と語釈。
...
1. 繁体字に無い鍵は**簡体字**へ（`LANG_FALLBACK`）。日本語へ落とすより読める
2. それも無ければ**日本語の語釈**をそのまま出す
3. そのときは語釈の右に**辞書へのリンク（↗）**を付ける（英語は jisho.org、他は Google 翻訳）

`t(key, {変数})` が画面の文字を引き、日本語のこども版だけ `jaKids` がやさしい言い換えを返す。
語釈は `meaningOf(a, round)`。鍵は答えのかなだが、**同じかなが別のラウンドに
別の語釈で入っている語**（〇ん〇ん の「しんしん＝雪が静かに降る」と 〇んしん の
「しんしん＝心身」）だけは `テンプレート + タブ + かな` を鍵にし、そちらを先に見る。
...
ステージは **氷山の層** で、進むほど下へ潜っていく。`氷の頂 +40m` から始まり、
`水面 0m` を越えて `藻場 −80m`、`無光層 −1,200m`、`海溝の口 −7,200m` と沈み、
`ことばの底 −11,000m` まで。

これは飾りではない。**ラウンドは正解数の多い順に並んでいる**ので、進むほど
正解が減り、見慣れない語ばかりになる。「水の上は誰でも知っている・下ほど数が
減って奇妙になる」という氷山の形は、この並び順そのものである。
以前はステージ名を日本の都市（札幌→那覇）にしていたが、札幌と那覇のあいだに
**難しくなる理由が無く**、移動が飾りに見えていた。深さに変えると、並び順が
そのまま物語になり、名前がいくつあっても足りる。

氷山は9層目 `氷の底 −200m` で終わる（実際の氷山は深くても200m台までしか
沈んでいない）。その先は氷を離れた海で、絵も `--keel` から下は暗く沈めてある。
**見えていたのは一角で、ことばはまだ続く**、という筋になっている。

- 層の名前と深さは [js/game.js](js/game.js) の `STAGE_LAYERS`（`[おとな版, こども版, 深さm]`）
- ステージ一覧は縦に積んだ氷山。`--water`（水面）と `--keel`（氷の終わり）は
  層の数から `seaLevelRatio()` `keelRatio()` が出す
- ステージを越えるときは、都市間を走るのではなく **下へ沈む**。深さの数字を数え下ろす
...
**スタート画面**は、いきなり層の一覧が開くと何の遊びで自分がどこにいるのかが
分からないために置いた。小さな氷山にいまいる深さを点で示し、主ボタンは
**前回の続き**を指す。共有された URL（`#s6&r=…`）で来たときだけ、この画面を
挟まずに指されたラウンドへ入る。

ラウンドをクリアしても**次へは自動で進まない**。一覧に戻り、いま片づけた札が
黒から白へ**判をつくように変わる**（`justCleared` → `stampIn`）。この層に
あと何問あるかを「のこり N 問」で見せ、次は本人に選ばせる。

URL は**いまどの画面にいるか**を持つ。読み込み直しても同じ場所に戻る。
...
ただし `r=` の指すラウンドが**クリア済み・降参済みなら、スタート画面に戻す**。
済んだ盤面に戻しても押せるかなが無く、手が止まるため。
...
| [js/i18n.js](js/i18n.js) | 画面の文字（日・英・中〈簡繁〉・越・韓） |
| [js/meanings-i18n.js](js/meanings-i18n.js) | 語釈の訳（あるぶんだけ。無い語は日本語のまま） |
