...
入り口の3枚目「For learners」から **English / 中文 / Tiếng Việt / 한국어** を選べる。
言語はモードとは別で、⚙メニューとホームでいつでも変えられる（`maruanagame-lang`）。

**訳すのは画面の文字だけ**。お題・答え・表記は日本語のままで、そこがこの遊びの中身。

| | 置き場所 | いまの状態 |
|---|---|---|
| 画面の文字 | [js/i18n.js](js/i18n.js) | 108鍵 × 4言語。層の名前30と段位9も |
| 語釈 | [js/meanings-i18n.js](js/meanings-i18n.js) | ROUND 01 の64語だけ。続きは [meanings-i18n.md](meanings-i18n.md) |

語釈は**訳のある語だけ差し替える**作り。無ければ日本語の語釈がそのまま出て、
右に辞書へのリンク（↗）が付く。だから途中まででも壊れない。
`t(key, {変数})` が鍵を引き、日本語のこども版だけ `jaKids` がやさしい言い換えを返す。
...
| [js/i18n.js](js/i18n.js) | 画面の文字（日・英・中・越・韓） |
